...
//    }
    public function form_add(){
        ...
        $post_error = parameter_check(['f_shop','f_name','f_phone','f_weight','f_pass_check','f_id_card','f_order_sn','f_address'],1);
        if($post_error['code'] == 300){
            return $post_error;
        }
//        return return_info(200,'Congratulations！',$post_error);
        $data = $post_error['data'];
        $data['f_api'] = '有赞';//来源
        $data['f_passport'] = input('post.f_passport');//护照（备选填写）
        $data['f_remark'] = input('post.f_remark'); //备注
